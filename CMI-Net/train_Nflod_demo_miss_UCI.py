...
from model_UCI.Polyloss import PolyLoss
import model.metric as module_metric
# import model.model_CNN_VIT1 as module_arch  # model_EEG_CNNvit_Orthogv2
import model_UCI.model_miss_Trans_GRU_UCI as module_arch  # model_EEG_CNNvit_Orthogv2
from parse_config import ConfigParser
from trainer.trainer_old_miss_UCI import Trainer
...
def main(config, fold_id):
    ...
    data_loader, valid_data_loader, data_count, train_data_count = data_generator_np_miss_UCI(folds_data[fold_id][0],
                                                                                     folds_data[fold_id][1], batch_size)

    weights_for_each_class = calc_class_weightUCI(data_count)
    ...
    criterion_Miss_MSE_acc = MSE_KLD_Loss()
    criterion_Miss_MSE_gyro = MSE_KLD_Loss()
    criterion_Miss_MSE_tot_acc = MSE_KLD_Loss()
    ...
    Miss_MSEacc_trainable_params = filter(lambda p: p.requires_grad, model.parameters())
    Miss_MSEgyro_trainable_params = filter(lambda p: p.requires_grad, model.parameters())
    Miss_MSEtot_acc_trainable_params = filter(lambda p: p.requires_grad, model.parameters())
    ...
    Miss_MSEacc_optimizer = config.init_obj('Miss_MSEacc_optimizer', torch.optim, Miss_MSEacc_trainable_params)
    Miss_MSEgyro_optimizer = config.init_obj('Miss_MSEgyro_optimizer', torch.optim, Miss_MSEgyro_trainable_params)
    Miss_MSEtot_acc_optimizer = config.init_obj('Miss_MSEtot_acc_optimizer', torch.optim, Miss_MSEtot_acc_trainable_params)
    ...
    trainer = Trainer(model, criterion,  metrics,
                      ...
                      class_weights=weights_for_each_class)
    """
    trainer = Trainer(model, criterion, criterion_Miss_SC, criterion_Miss_MSE_acc, criterion_Miss_MSE_gyro, criterion_Miss_MSE_tot_acc, metrics,
                      optimizer,
                      config=config,
                      data_loader=data_loader,
                      fold_id=fold_id,
...
if __name__ == '__main__':
    ...
    if "shhs" in args2.np_data_dir:
        ...
        folds_data = load_folds_data_UCI(args2.np_data_dir, config["data_loader"]["args"]["num_folds"])
